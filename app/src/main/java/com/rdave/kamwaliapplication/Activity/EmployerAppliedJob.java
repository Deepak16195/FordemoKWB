...
public class EmployerAppliedJob extends AppCompatActivity {
...
AppliedJobLAdapter appliedJobLAdapter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        initView();
        LoginedAs = SharePrefarence.getmInstance(EmployerAppliedJob.this).getLoginAs();
        Logined = SharePrefarence.getmInstance(EmployerAppliedJob.this).getLogined();
        Username = SharePrefarence.getmInstance(EmployerAppliedJob.this).getUserName();
        Useremail = SharePrefarence.getmInstance(EmployerAppliedJob.this).getEmailid();
        UserID = SharePrefarence.getmInstance(EmployerAppliedJob.this).getUser_Id();

        if (Logined.equalsIgnoreCase("1"))
        {
            if (LoginedAs.equalsIgnoreCase("Candidate")) {
                Utils.customMessage(EmployerAppliedJob.this,"You Are Not Eligible For This Feature");
                finish();
    ...
    private void getAppliedJob(String UserID) {
        Utils.showProgressDialog(EmployerAppliedJob.this);
        ApiUtils.getAPIService().getAppliedJob(UserID).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<AppliedJob>() {
                    ...
                    public void onError(Throwable e) {
                        Utils.hideProgressDialog();
                        Utils.customMessage(EmployerAppliedJob.this, e.getMessage().toString());
                    }
                        ...
                        if (currencyModel.isSuccess()) {
                            if (!currencyModel.getMessage().equalsIgnoreCase("Server Error!")) {
                                if (currencyModel.getMessage().equalsIgnoreCase("Success")) {
                                    ...
                                    appliedJobLAdapter = new AppliedJobLAdapter(EmployerAppliedJob.this, mAppliedJobs);
                                    hRecyclerview.setAdapter(appliedJobLAdapter);
                                } else {
                                    Utils.custoAlert(EmployerAppliedJob.this, currencyModel.getMessage());
                                }
                            }
                        }
                    }
